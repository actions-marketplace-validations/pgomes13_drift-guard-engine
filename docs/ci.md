...
Use drift-bot directly in CI to fail a PR when breaking API changes are detected.
...
jobs:
  drift:
    ...
    steps:
      ...
      - name: Install drift-bot
        run: |
          curl -sSL https://github.com/DriftBot/api-drift-engine/releases/latest/download/drift-bot_linux_amd64.tar.gz | tar -xz
          sudo mv drift-bot /usr/local/bin/

      - name: Check for breaking changes
        run: drift-bot compare --fail-on-breaking
```
...
- name: Diff schemas
  id: diff
  run: |
    drift-bot compare --format json > drift-diff.json
...
drift-bot is a single static binary — install it the same way on any CI runner.
