...
func tempDir(t *testing.T) string {
	t.Helper()
	dir, err := os.MkdirTemp("", "drift-guard-files-*")
	if err != nil {
...
func TestFindSwaggerScript_DriftGuardScriptsTS(t *testing.T) {
	dir := tempDir(t)
	touch(t, dir, "drift-guard/scripts/generate-swagger.ts")
	got := findSwaggerScript(dir)
	if got != "drift-guard/scripts/generate-swagger.ts" {
		t.Errorf("expected drift-guard/scripts/generate-swagger.ts, got %q", got)
	}
}
...
func TestFindSwaggerScript_PrefersFirstMatch(t *testing.T) {
	dir := tempDir(t)
	// Both present — drift-guard/scripts/ should win (it's listed first).
	touch(t, dir, "drift-guard/scripts/generate-swagger.ts")
	touch(t, dir, "generate-swagger.ts")
	got := findSwaggerScript(dir)
	if got != "drift-guard/scripts/generate-swagger.ts" {
...
func TestCopyFile_CopiesContent(t *testing.T) {
	...
	if err := os.WriteFile(src, []byte("hello drift-guard"), 0o644); err != nil {
	...
	if string(got) != "hello drift-guard" {
		t.Errorf("expected 'hello drift-guard', got %q", string(got))
	}
}
